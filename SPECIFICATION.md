...
**複数料理対応**: 1枚の写真に複数の料理が写っている場合、ChatGPTがオブジェクトの配列
（`[{...}, {...}]`）で返すことがある。単一オブジェクトと配列の両方を受け付け、配列の場合は
各要素を個別に検証したうえで、全件が有効な場合のみ複数の `meals` 行としてまとめて登録する
（all-or-nothing。1件でも不正な要素があれば `VALIDATION_ERROR` とし、`details[].path` に
`[インデックス].フィールド名` 形式でどの要素のどの項目が誤りかを示す）。

**処理フロー**:

1. 認証解決（セッション → `user_id`）
2. `request.json()` パース（失敗 → `INVALID_JSON`）
3. 配列かどうかを判定し、各要素をZodスキーマ検証（1件でも失敗 → `VALIDATION_ERROR` + details）
4. 検証済みの各要素について `meals` へINSERT（`raw_json` には該当要素の原文を格納）
...
```json
{ "success": true, "id": "xxxx-uuid", "ids": ["xxxx-uuid"], "count": 1 }
```

配列入力で複数登録した場合は `ids` に全件のIDが入り、`count` が件数になる。`id` は後方互換のため
先頭要素のIDを返す。
...
### 8.2 食事登録画面（`/import`）— Phase 1の中核

**入力モード切替タブ**: 「JSON貼り付け」「手入力」。写真を撮れなかった場合や、ChatGPT解析を
経由せず直接記録したい場合のために手入力モードを用意する（`MealManualForm`コンポーネント）。
手入力モードは日付・時刻・食事区分・食事タイミング・料理名・ブランド（任意）・カテゴリ・分量・
カロリー・PFC・食物繊維・塩分・食材（カンマ区切り）を個別フィールドで入力するフォームで、
`recognition_type` は「一般料理」固定（AIによる商品/推定の判別が発生しないため）、
`evaluation`/`advice`/`confidence` はAI解析結果専用の項目のため空のまま送信する。
送信先は`POST /api/meals`で、JSON貼り付けと完全に同じAPIを利用する（6.2節）。

以下はJSON貼り付けモード（`MealJsonImport`コンポーネント）の仕様:

| 要素 | 仕様 |
|---|---|
| JSON入力欄 | `<textarea>` 大きめ（10行以上）。等幅フォント。プレースホルダに例JSONを薄く表示 |
| 貼付ボタン | `navigator.clipboard.readText()` でワンタップ貼付（権限エラー時は手動貼付を案内） |
| 解析 | 入力変化時に自動でパース＋Zod検証（デバウンス300ms）。単一オブジェクト・配列（複数料理）の両方に対応 |
| バリデーション表示 | エラー時: フィールド単位のエラーリストを赤枠で表示（配列の場合は `[インデックス].フィールド名`）。登録ボタン無効化 |
| プレビュー | 検証成功時にカード表示（下記）。配列の場合は件数分のカードを縦に並べて表示 |
| 登録ボタン | プレビュー確認後にタップ → `POST /api/meals` → 成功トースト（「N件の登録が完了しました」）→ 入力欄クリア → 「履歴を見る」導線 |
...
- **日付ナビゲーション（DateNav）**: 画面上部に前日/翌日ボタンと、📅アイコン付きの明示的なボタンを備え、
  `?date=YYYY-MM-DD` クエリで過去日のサマリーも同じレイアウトで閲覧できる。未来日は選択不可。
  ネイティブ`<input type="date">`は日毎の情報を表示できないため使用せず、カスタムカレンダー
  （`DateCalendar`コンポーネント）をモーダル表示する。
- **カスタムカレンダー（DateCalendar）**: 月表示のカレンダーグリッド。前月/翌月ボタンで移動。
  `GET /api/daily-summaries?from=&to=` で表示月分の `daily_summaries` とユーザーの目標値をまとめて取得し、
  各日のマスに「8.10節のスコア計算」による100点満点のスコアバッジ（80点以上=緑、50〜79点=黄、
  50点未満=赤）を表示する。記録が無い日はスコア非表示。日付タップで選択・遷移。
...
カレンダー（8.3節）に表示する、その日の食事記録が目標にどれだけ沿っていたかを示す100点満点のスコア。
`src/lib/utils/score.ts` の `calculateDailyScore()` で算出し、記録が無い日（`meal_count === 0`）は
`null`（非表示）。加重平均の内訳:

| 項目 | 重み | 満点条件 | 減点条件 |
|---|---|---|---|
| カロリー | 40% | 目標の70%〜100%の範囲内 | 超過（超過率×150で減点）/ 70%未満（不足率×100で減点） |
| タンパク質 | 25% | 目標以上 | 目標未満は `実績/目標×100` |
| 脂質 | 15% | 目標以下 | 超過率×100で減点 |
| 炭水化物 | 10% | 目標以下 | 超過率×100で減点 |
| 塩分 | 10% | 目標以下 | 超過率×100で減点 |

`GET /api/daily-summaries?from=&to=` が指定期間の `daily_summaries` とユーザーの目標値をまとめて返し、
クライアント側（`DateCalendar`）でこの関数を呼び出してカレンダー各日にスコアバッジを表示する。
